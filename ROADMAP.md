...
| --- | --- | --- | --- |
| 2.1 | **Customers & WhatsApp lists** — a private customer list per business (name, number, tags, "agreed to receive messages" tick), and one-tap **click-to-chat** WhatsApp links with a ready message (festival offer, thank-you, review request). The client taps each link themselves; nothing is sent automatically, so there is no spam or WhatsApp-ban risk. | `Next` | Nothing. Only people marked as having agreed are listed for messages. |
...
| 2.5 | **Festival & occasion calendar** — India's festivals and business occasions pre-loaded, so the planner suggests "Diwali gifting post — plan it now" ahead of time. | `Next` | Nothing. The festival style already exists in Creative Studio. |
...
1. Run `4-RUN-creative-studio-migration.sql` (adds the AI-photo usage log, migration 0037) — and, when it is delivered, the file that adds the picture-description column (0038) and any later migrations. Paste the **contents** of each file into the Supabase SQL Editor, not the file name.
