...
`0001` → `0039` in `supabase/migrations/`. The latest seven (`0033` Buffer insights, `0034` protected `client_settings` columns, `0035` cron log, `0036` AI-answer checks, `0037` Creative Studio usage log, `0038` the "Picture idea" column, `0039` the customer list) are safe to re-run. Until `0039` is applied, the Customers & WhatsApp page shows "not set up yet" and hides its forms. Until `0037` is applied, AI photos are refused (they fail closed, because their daily usage can't be counted); brand-colour and stock-photo graphics still work. Until `0038` is applied, a picture description still works for the graphic being made, but it isn't remembered on the post (every write that includes the column falls back to the old behaviour on a database without it).
