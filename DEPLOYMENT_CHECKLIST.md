...
After the migrations, run `supabase/tests/tenant_isolation_test.sql` **on its own** (it ends in a deliberate error whose text is the report): expect `PASS`, and the admin-column line to read `PROTECTED`. This is the empirical two-client isolation check; run it again after any change to policies.
