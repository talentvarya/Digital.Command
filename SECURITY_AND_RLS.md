...
`provider` (`0019_ai_provider_tracking.sql`, post-Phase 7, added once Kimi became a second AI option) needed **no RLS change** — both policies above are plain row policies with no column list, so a new column is automatically covered by whatever access the row already has. Confirmed by reading this table's policy definitions before writing that migration, not assumed.
...
- `MOONSHOT_API_KEY` (post-Phase 7) — server-only; read only inside `lib/ai/providers/kimi.ts`. Optional — only required when `AI_PROVIDER=kimi`; the app runs fine without it (default provider is Anthropic). Same sensitivity class as `ANTHROPIC_API_KEY`.
