...
### Phase 10–12, audit-pass and Creative Studio tables (`0028`, `0030`, `0032`, `0035`, `0036`, `0037`)
...
- **`creative_generations`** (`0037`, Creative Studio usage log) — org `SELECT` on their own rows and Super Admin `SELECT`, and **no insert, update or delete policy for any logged-in role**. Only server code holding the service-role key writes it. That is deliberate: its `ai_photo` rows are what keep the shared FREE Cloudflare allowance from being used up, so if a client could insert rows they could fake usage and block every other client's AI photos. The quota check fails closed — if the rows can't be counted, no AI photo is made.

### Customers' contact details (`0039_customer_contacts.sql`)

`customer_contacts` holds a business's own customers' names and WhatsApp numbers, so it is the most personal data in the product and is treated more strictly than the other tables: **members of that business can read, add, change and delete their own rows — and nobody else can, including a Super Admin.** Unlike almost every other table there is deliberately no `or is_super_admin()` in the read policy, because support has no need to see a client's customers. This is checked on every push by a test of its own (`tests/rls-isolation.test.ts`, "customer contacts privacy"): another business can neither read, insert into, update nor delete these rows, and a Super Admin session reads none of them.

Around it: numbers are stored as digits with the country code; a customer counts as messageable only with `consent = true` and `opted_out = false` (the lists in the app never include anyone else, and marking a message as opened is refused for them); the audit trail records that a customer was added, imported or removed but **never their name or number**; and the app never sends anything — each WhatsApp message is opened in WhatsApp by the client, who presses send. There is no database-level format check on `phone`, because the tenant-isolation harness seeds text columns generically; the format is enforced in the server actions (`lib/contacts/phone.ts`).
...
- `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_AI_API_TOKEN` (optional) — server-only; read only inside `lib/creative/cloudflare.ts`. One app-wide Cloudflare account (not per client) powers the free AI photos. Create the token with the "Workers AI" permission only, so a leak can't touch anything else in the account. Error messages shown to clients never contain either value or the variable names.
